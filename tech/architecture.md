...
> **重要**：基于 ADR 004，Workbench 的定位已明确为**主要是内部运营平台**，而非对外的 SaaS 产品。

**主要定位（90%）**：内部运营平台
- Univers 团队用来管理 AI 执行者（Operation层）
- 高效管理设备、数据、AI 工作流
- 支持同时为多个客户提供服务
- 让服务团队能够规模化服务能力
...
**核心作用**：
```
Univers 团队 → 使用 Workbench → 管理多个 AI 执行者
       ↓
同时服务多个客户 → 客户购买结果 → 不需要使用 Workbench

类比：
- Uber 的调度系统（内部工具，规模化服务）
- McKinsey 的知识库（内部工具，提升服务质量）
```

**技术定位**：通用 AIoT 基础设施平台 + 人机协作工作台
