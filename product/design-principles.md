...
**理念**：人类的独特能力是**上下文自由缩放**（见 [核心哲学](../PHILOSOPHY.md)），产品要支持用户在多个尺度上思考

**为什么重要**：
- 价值判断需要在多个尺度上权衡
- 战略思考需要跨时间、空间、抽象层级
- AI 在固定框架内优化，人类选择框架和尺度
...
```
┌─────────────────────────────────────────────┐
│ 时间尺度： [当前] [今日] [本周] [本月] [本年] [长期] │
│ 空间尺度： [此设备] [此区域] [此楼] [全部] [行业]    │
│ 抽象层级： [数据] [操作] [策略] [目标] [愿景]      │
├─────────────────────────────────────────────┤
│ 当前视图：本月 × 此楼 × 策略层                │
│                                              │
...
├──────────────┼──────────────┼──────────────┤
│ 3F 温度:     │ 3F 能效:     │ 3F 战略:     │
│ 24°C (正常)  │ 节能 12%     │ 满意度 85%   │
│              │              │              │
│ AI 决策 15次 │ AI 决策 456次│ AI 学习曲线  │
│ 人工干预 0次 │ 人工干预 8次 │ 持续改进 ✓   │
│              │              │              │
│ ✓ 一切正常   │ ✓ 达到目标   │ ⚠️ 需调整方向│
└──────────────┴──────────────┴──────────────┘
```
...
让高级用户快速切换：
- `Alt+T`：切换时间尺度（当前 → 今日 → 本月 → 本年）
- `Alt+S`：切换空间尺度（设备 → 区域 → 楼宇 → 全部）
- `Alt+L`：切换抽象层级（数据 → 操作 → 策略 → 目标）
- `Alt+?`：显示"这个决策在其他尺度上如何？"

#### 设计要点

1. **默认合适尺度，支持自由缩放**
   - 根据任务选择默认尺度（设定目标用战略尺度，查看状态用实时尺度）
   - 但随时可以切换

2. **保持上下文**
   - 切换尺度时，保持焦点（从"今日 3F 温度"缩放到"本月 3F 能效"）
   - 提供"回到之前的视图"功能
